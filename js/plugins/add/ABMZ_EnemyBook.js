...
*/
		...
		// v1.30 (DungeonEscape改変: 重複スキルをまとめて表示するよう修正)
		var uniqueSkillIds = [];
		for (var i=0; i<dataEnemy.actions.length; i++) {
			var action = dataEnemy.actions[i];
			if (action && !uniqueSkillIds.includes(action.skillId)) {
				uniqueSkillIds.push(action.skillId);
			}
		}
		for (var i=0; i<DispSkillNumber; i++) {
			var skillId = uniqueSkillIds[i];
			if (skillId) {
				if (!isHideStatus) {
					this.drawItemName($dataSkills[skillId], x, y, columnWidth);
					y += lineHeight;
				} else {
