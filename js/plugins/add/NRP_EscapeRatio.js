...
BattleManager.makeEscapeRatio = function() {
    ...
    // ▼デバッグ用(確認後に削除してください/正しいタイミングはここ)
    console.log("EscapeRatio debug: 計算直後の逃走成功率=", this._escapeRatio,
        "(" + Math.round(this._escapeRatio * 100) + "%)",
        "出現中の敵の数=", $gameTroop.aliveMembers().filter(e => e.isAppeared()).length);
};
...
// ----------------------------------------------------------------------------
// デバッグ用(確認後にこのブロックごと削除してください)
// ----------------------------------------------------------------------------

const _BattleManager_processEscape_debug = BattleManager.processEscape;
BattleManager.processEscape = function() {
    const result = _BattleManager_processEscape_debug.apply(this, arguments);
    console.log("EscapeRatio debug: 結果=", result ? "成功" : "失敗");
    return result;
};
